...
The server will run a challenge-response protocol using the hint generated by the sender on message submission. The protocol ensures that the receivers only receive ciphertexts sent to them and the server does not learn which client the message was intended for.
