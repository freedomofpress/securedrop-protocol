...
let impl_ServerStorage__deterministic_uuid
      (#v_R: Type0)
      (#[FStar.Tactics.Typeclasses.tcresolve ()] i0: Rand_core.t_RngCore v_R)
      (#[FStar.Tactics.Typeclasses.tcresolve ()] i1: Rand_core.t_CryptoRng v_R)
      (self: t_ServerStorage)
      (rng: v_R)
    : (t_ServerStorage & v_R & Uuid.t_Uuid) =
  let bytes:t_Array u8 (mk_usize 16) = Rust_primitives.Hax.repeat (mk_u8 0) (mk_usize 16) in
  let (tmp0: v_R), (tmp1: t_Array u8 (mk_usize 16)) =
    Rand_core.f_fill_bytes #v_R #FStar.Tactics.Typeclasses.solve rng bytes
  in
  let rng:v_R = tmp0 in
  let bytes:t_Array u8 (mk_usize 16) = tmp1 in
  let _:Prims.unit = () in
  let hax_temp_output:Uuid.t_Uuid =
    Uuid.Builder.impl_Builder__into_uuid (Uuid.Builder.impl_Builder__from_random_bytes bytes
        <:
        Uuid.Builder.t_Builder)
  in
  self, rng, hax_temp_output <: (t_ServerStorage & v_R & Uuid.t_Uuid)
