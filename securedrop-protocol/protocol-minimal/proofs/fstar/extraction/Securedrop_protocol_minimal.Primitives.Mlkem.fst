module Securedrop_protocol_minimal.Primitives.Mlkem
...
let from_bytes (seed: t_Array u8 (mk_usize 64))
  ...
  let mlkem_encaps:t_MLKEM768PublicKey =
    impl_MLKEM768PublicKey__from_bytes (Libcrux_ml_kem.Types.impl_23__as_slice (mk_usize 1184) pk
        <:
        t_Array u8 (mk_usize 1184))
  in
  let mlkem_decaps:t_MLKEM768PrivateKey =
    impl_MLKEM768PrivateKey__from_bytes (Libcrux_ml_kem.Types.impl_15__as_slice (mk_usize 2400) sk
        <:
...
let typed (sk: Libcrux_kem.t_PrivateKey) (pk: Libcrux_kem.t_PublicKey)
  ...
  then
    ...
    | Core_models.Result.Result_Ok hoist9 ->
      let private_key:t_MLKEM768PrivateKey = impl_MLKEM768PrivateKey__from_bytes hoist9 in
      (match
        ...
        | Core_models.Result.Result_Ok hoist10 ->
          let public_key:t_MLKEM768PublicKey = impl_MLKEM768PublicKey__from_bytes hoist10 in
          Core_models.Result.Result_Ok
...
let deterministic_keygen (randomness: t_Array u8 (mk_usize 64))
    : Core_models.Result.t_Result (t_MLKEM768PrivateKey & t_MLKEM768PublicKey) Anyhow.t_Error =
  match
...
let generate_mlkem768_keypair
  ...
  | Core_models.Result.Result_Ok (sk, pk) ->
    let hax_temp_output:Core_models.Result.t_Result (t_MLKEM768PrivateKey & t_MLKEM768PublicKey)
      Anyhow.t_Error =
      typed sk pk
    in
    rng, hax_temp_output
