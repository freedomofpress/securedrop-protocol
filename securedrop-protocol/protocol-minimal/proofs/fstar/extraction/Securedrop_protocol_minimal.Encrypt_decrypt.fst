...
let encrypt
  ...
  (out:
    ...
    Securedrop_protocol_minimal.Message.auth_enc #v_R
      ...
      (Core_models.Ops.Deref.f_deref #(Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
          #FStar.Tactics.Typeclasses.solve
          (Securedrop_protocol_minimal.Ciphertext.impl_Plaintext__to_bytes plaintext
            <:
  ...
  let sender_apke_bytes:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    Securedrop_protocol_minimal.Message.impl_MessagePublicKey__as_bytes (Securedrop_protocol_minimal.Traits.f_message_auth_pk
          #v_Sender
          #FStar.Tactics.Typeclasses.solve
          sender
        <:
...
let decrypt
  ...
  (results:
    ...
    Alloc.Vec.impl__new #(Securedrop_protocol_minimal.Keys.t_MessageKeyBundle &
      ...
      ()
  in
  let results:Alloc.Vec.t_Vec
    (Securedrop_protocol_minimal.Keys.t_MessageKeyBundle & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
    Alloc.Alloc.t_Global =
    Core_models.Iter.Traits.Iterator.f_fold (Core_models.Iter.Traits.Collect.f_into_iter #(Alloc.Vec.t_Vec
              Securedrop_protocol_minimal.Keys.t_MessageKeyBundle Alloc.Alloc.t_Global)
          #FStar.Tactics.Typeclasses.solve
          (Securedrop_protocol_minimal.Traits.f_keybundles #v_U
              #FStar.Tactics.Typeclasses.solve
              receiver
            <:
        ...
        <:
        Alloc.Vec.Into_iter.t_IntoIter Securedrop_protocol_minimal.Keys.t_MessageKeyBundle
          Alloc.Alloc.t_Global)
      results
      (fun results bundle ->
          let results:Alloc.Vec.t_Vec
  ...
  (raw_metadata: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    Core_models.Option.impl__expect #(Securedrop_protocol_minimal.Keys.t_MessageKeyBundle &
        Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
      (Core_models.Slice.impl__first #(Securedrop_protocol_minimal.Keys.t_MessageKeyBundle &
            Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
  ...
  let sender_pk:Securedrop_protocol_minimal.Message.t_MessagePublicKey =
    Core_models.Result.impl__expect #Securedrop_protocol_minimal.Message.t_MessagePublicKey
      #Anyhow.t_Error
      (Securedrop_protocol_minimal.Message.impl_MessagePublicKey__from_bytes (Core_models.Ops.Deref.f_deref
              #(Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
              #FStar.Tactics.Typeclasses.solve
              raw_metadata
            <:
  ...
  Core_models.Result.impl__unwrap #Securedrop_protocol_minimal.Ciphertext.t_Plaintext
    #Anyhow.t_Error
    (Securedrop_protocol_minimal.Ciphertext.impl_Plaintext__from_bytes (Core_models.Ops.Deref.f_deref
            #(Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
            #FStar.Tactics.Typeclasses.solve
            pt
          <:
      ...
      (fun message_ids chall ->
          ...
          | Core_models.Result.Result_Ok message_id_bytes ->
            let uuid:Uuid.t_Uuid =
              Core_models.Result.impl__expect #Uuid.t_Uuid
                #Uuid.Error.t_Error
                (Uuid.Builder.impl__from_slice (Core_models.Ops.Deref.f_deref #(Alloc.Vec.t_Vec u8
                            Alloc.Alloc.t_Global)
                        #FStar.Tactics.Typeclasses.solve
                        message_id_bytes
                      <:
                      t_Slice u8)
                  <:
                  Core_models.Result.t_Result Uuid.t_Uuid Uuid.Error.t_Error)
                "Need uuid from decrypted message_id_bytes"
            in
            let message_ids:Alloc.Vec.t_Vec Uuid.t_Uuid Alloc.Alloc.t_Global =
