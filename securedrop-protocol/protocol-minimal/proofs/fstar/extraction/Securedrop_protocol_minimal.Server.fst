...
let impl_Server__create_newsroom_setup_request
  ...
  (out:
    Core_models.Result.t_Result Securedrop_protocol_minimal.Keys.Newsroom.t_NewsroomKeyPair
      Anyhow.t_Error) =
    Securedrop_protocol_minimal.Keys.Newsroom.impl_NewsroomKeyPair__new #v_R rng
  in
  let rng:v_R = tmp0 in
  match
    out
...
let impl_Server__handle_message_submit
      ...
      (rng: v_R)
    : (t_Server & v_R & Core_models.Result.t_Result Uuid.t_Uuid Anyhow.t_Error) =
  let (tmp0: Securedrop_protocol_minimal.Storage.t_ServerStorage), (tmp1: v_R), (out: Uuid.t_Uuid) =
    Securedrop_protocol_minimal.Storage.impl_ServerStorage__deterministic_uuid #v_R
      self.f_storage
      rng
  in
  let self:t_Server = { self with f_storage = tmp0 } <: t_Server in
  let rng:v_R = tmp1 in
  let message_id:Uuid.t_Uuid = out in
  let self:t_Server =
    {
      self with
      f_storage
