...
// `encode()` returns a `Vec` whose length depends on the algorithm of the key.
// Algorithm-specific size guarantees are supplied as preconditions in the
// `typed()` wrapper functions in each primitive module, so no per-size
// refinement is expressed here.
