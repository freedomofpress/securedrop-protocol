...
# HPKE settings
HPKE_SUITE_MESSAGE = hybrid_pke.default(mode=hybrid_pke.Mode.AUTH_PSK)
HPKE_SUITE_METADATA = (
    hybrid_pke.default()
)  # todo: need library support for a hybrid KEM in HPKE Base (unauthenticated) mode
HPKE_INFO = b""
HPKE_AAD = b""
HPKE_PSK_ID = b"PSK_INFO_ID_TAG"  # Spec requires a tag

# What is this, a metadata parser for ants? Toy purposes only
LEN_DHKEM_ENCAPS = 32
...
def GenerateDHAKEMKeypair() -> Tuple[bytes, bytes]:
    """
    HPKE (DH-AKEM) keypair. Used for message encryption.
    """
    # (sk, pk)
    return HPKE_SUITE_MESSAGE.generate_key_pair()
...
def GeneratePQKEMKeypair():
    """
    PQ KEM (PSK shared secret) keypair.
    """
    encaps, decaps = ML_KEM_768.keygen()
    return (encaps, decaps)


def GenerateHybridKeypair():
    """
    TODO: eventual hybrid KEM (metadata seal) keypair.
    """
    return HPKE_SUITE_METADATA.generate_key_pair()
...
class User:
    def encrypt(
        self, plaintext: bytes, recipient: Union["Source", "Journalist"]
    ) -> "Envelope":
        """
        Encrypt a plaintext message to a given recipient using HPKE's AuthPSK mode with a PQ shared secret as the PSK. The encrypted message is not signed directly by the sender, but incorporates an ephemeral message key,which maintains message-level deniability while preserving sender
        authentication.

        For message decryption, in addition to the ciphertext, the receiver needs the following information, encrypted via an (unauthenticated, in future PQ/T hybrid) HPKE.seal() per RFC9180 section 9.9, Metadata Protection:

          * the sender's DH-AKEM pubkey
          * the encpasulation of the PQ shared secret key (PSK)
          * the (DH-AKEM, non-PQ) encapsulation of the DH-AKEM message shared secret

        Finally, a "Clue" (mgdh) is generated to allow the server to obliviously transfer messages for receiver.

        Return an Envelope, representing the entire payload to the sever, that contains:

          * the authenticated+sealed message ciphertext
          * the sealed metadata
          * the encapsulation of the metadata shared secret
          * the two parts of the mgdh/"clue" (ephemeral DH pubkey + mgdh)

        Note: per <https://datatracker.ietf.org/doc/html/rfc9180#name-hiding-plaintext-length>, padding is an application-level responsibility.
        """
        # PQ shared secret key for PSK, and its encapsulation.
        # Note: HPKE spec requires at minimum 32 byte shared secret, and we
        # want a uniformly random secret. MLKEM outputs a 32 byte shared secret
        # key, i.e. an output that can be used directly as a secret key.
        pq_ss, pq_ss_enc = ML_KEM_768.encaps(recipient.get_pqkem_encaps())

        # HPKEAuthPSK mode encrypt the message. The recipient pubkey
        # is standard (non PQ) DH-AKEM.
        # TODO: structured Message object that includes reply keys/signatures
        # TODO/discuss: per @lumaier, source and journo construct different format messages
        dhkem_ss_encaps, message_ct = HPKE_SUITE_MESSAGE.seal(
            recipient.get_dh_pk(),
            HPKE_INFO,
            HPKE_AAD,
            plaintext,
        ...
        # HPKE.Base mode encrypt metadata needed for message decryption (9.9).
        # TODO: Use a PQ/T hyrid KEM (not yet supported by HPKE libraries).
        metadata_encap, metadata_ct = HPKE_SUITE_METADATA.seal(
            recipient.get_metadata_encaps(), HPKE_INFO, HPKE_AAD, metadata_bytes
        )

        # Build message clue (mgdh)
        ...
        return Envelope(
            cmessage=message_ct,
            cmetadata=metadata_ct,
            metadata_encap=metadata_encap,
            mgdh_pubkey=pk_gdh,
            mgdh=mgdh,
        )
    ...
    def decrypt(self, envelope: "Envelope") -> "Plaintext":
        # Decaps metadata
        metadata_untrusted = HPKE_SUITE_METADATA.open(
            envelope.metadata_encap,
            self._get_metadata_decaps(),
            HPKE_INFO,
            HPKE_AAD,
            envelope.cmetadata,
        )
        ...
        shared_pqkem_encap_untrusted = metadata_untrusted[
            LEN_DHKEM_ENCAPS : (LEN_DHKEM_ENCAPS + LEN_PQKEM_ENCAPS)
        ]
        sender_pkey_bytes_untrusted = metadata_untrusted[-1 * LEN_SENDER_DH_KEY :]

        # Decaps PQ KEM to get shared secret, used as PSK
        shared_secret_pqkem_untrusted = ML_KEM_768.decaps(
            self._get_pqkem_decaps(), shared_pqkem_encap_untrusted
        )

        # Authenticate and decrypt message
        message_bytes_untrusted = HPKE_SUITE_MESSAGE.open(
            shared_dhkem_encap_untrusted,
            self._get_dh_sk(),
            HPKE_INFO,
            HPKE_AAD,
            envelope.cmessage,
            psk=shared_secret_pqkem_untrusted,
            psk_id=HPKE_PSK_ID,
            pk_s=sender_pkey_bytes_untrusted,
        )
    ...
    def get_pqkem_encaps(self):
        """
        PQ KEM encaps key used to derive psk in AuthPSK mode.
        """
        raise NotImplementedError("child must implement!")
    ...
    def get_metadata_encaps(self):
        """
        PKE (Metadata seal). This should be a hybrid (PQ/T) KEM (TODO: yes? discuss.)
        """
        raise NotImplementedError("child must implement!")
...
class Journalist(User):
    def __init__(self, newsroom: Newsroom):
        ...
        self._je_sk_metadata, self._je_pk_metadata = GenerateHybridKeypair()
    ...
    def get_metadata_encaps(self):
        # Should be hybrid kem, used for unauthenticated HPKE.seal()
...
class Source(User):
    def __init__(self):
        ...
        self._sk_metadata, self._pk_metadata = GenerateHybridKeypair()
...
class Plaintext:
    def __init__(
        self, msg: bytes, sender_key: bytes, recipient_reply_key: Optional[bytes] = None
    ):
        self.msg = msg
...
class Envelope:
    ...
    def __init__(
        ...
        cmessage: bytes,
        cmetadata: bytes,
        metadata_encap: bytes,
        mgdh_pubkey: bytes,
        mgdh: bytes,
    ):
        self.cmessage = cmessage
        # Encrypted metadata contains 3 things: DH-AKEM ss encaps, PQ-KEM ss encaps, and sender key
...
def main():
    ...
    envelope3 = journalist.encrypt(message3_in, journalist2)
    ...
    envelope4 = source.encrypt(message4_in, source2)
