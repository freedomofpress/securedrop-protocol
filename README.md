...
- Journalist key material generated by `pki.py` is stored on-disk under `keys/`. It is accessed there by the journalist client, which uploads public keys to the server
...
* **Message expiry/deletion will occur on a fuzzy interval.** 
  ...
  Client-side (local) message deletion will be supported for journalists. Note this is not an anti-forensic measure, because some indicator (such as a uuid hash) will be retained in order to prevent re-downloading a locally-deleted message.

* **Messaging an arbitrary subset of journalists will not be supported in the UI.**
